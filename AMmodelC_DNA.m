...
for k=1:500
% delta_alpha=A(randi(numel(A),1,1))/360*2*pi;
...
x_axis4_Spin1=M-new_C1a(3,:);
x_axis4_Spin1=x_axis4_Spin1/norm(x_axis4_Spin1);
M=new_C1a(3,:)+x_axis4_Spin1*11.4;
...
x_axis3_Spin2(i,:)=x_axis2_Spin2(i,:).*cos(beta_2(i))-new_zaxisB(7+(i-1),:).*sin(beta_2(i));
%%find Spin2
...
M2(i,:)=new_C1b(7+(i-1),:)+x_axis4_Spin2(i,:).*11.4;
end
...
rmsd(:,o)=sqrt(sum((R-Distance_4Punkte.*10).^2,2)./500);
Distance=R./10;
for w=1:10
bpd=w;
